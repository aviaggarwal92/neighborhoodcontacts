...
var TursoDatabaseServerlessSession = class TursoDatabaseServerlessSession extends SQLiteAsyncSession {
	...
	async transaction(transaction, config) {
		let clientTx = this.client.transactionAsync(async (txClient) => {
			const session = new TursoDatabaseServerlessSession(txClient, this.dialect, this.relations, this.options);
			return await transaction(new TursoDatabaseServerlessTransaction("async", this.dialect, session, this.relations));
		});
		if (config?.behavior) clientTx = clientTx[config.behavior];
		return clientTx();
	}
};
