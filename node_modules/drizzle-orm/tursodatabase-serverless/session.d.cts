...
import { Connection, Transaction } from "@tursodatabase/serverless";
...
declare class TursoDatabaseServerlessSession<TRelations extends AnyRelations> extends SQLiteAsyncSession<'async', TursoDatabaseServerlessRunResult, TRelations> {
  ...
  private logger;
  private cache;
  constructor(client: Connection | Transaction, dialect: SQLiteDialect, relations: TRelations, options: TursoDatabaseServerlessSessionOptions);
  ...
  transaction<T>(transaction: (db: TursoDatabaseServerlessTransaction<TRelations>) => Promise<T>, config?: SQLiteTransactionConfig): Promise<T>;
}
