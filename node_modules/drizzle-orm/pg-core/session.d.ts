...
type PgTransactionConfig = {
  ...
  deferrable?: boolean | undefined; /** `snapshot` cannot be used with `isolationLevel: 'read uncommitted' | 'read committed'`  */
  snapshot?: undefined;
} | {
  /** `snapshot` cannot be used with `isolationLevel: 'read uncommitted' | 'read committed'`  */isolationLevel?: 'repeatable read' | 'serializable' | undefined;
  accessMode?: 'read only' | 'read write' | undefined;
...
declare abstract class PgSession {
  ...
  abstract prepareQuery(query: Query, mode: 'arrays' | 'objects' | 'raw', name: string | boolean, mapper: ((rows: unknown[]) => any) | undefined, queryMetadata?: {
    ...
    tables: string[];
  }, cacheConfig?: WithCacheConfig, /** Usef for driver-side mapping if supported by driver & compatible */

  driverShape?: unknown): PgBasePreparedQuery;
