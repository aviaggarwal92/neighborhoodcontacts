...
var PgEffectTransaction = class extends require_pg_core_effect_db.PgEffectDatabase {
	...
	getTransactionConfigSQL(config) {
		return __sql_sql_ts.sql.raw(this.getTransactionConfigChunks(config).join(" "));
	}
	/** @internal */
	setTransactionSnapshotSQL(snapshot) {
		return __sql_sql_ts.sql`set transaction snapshot ${snapshot}`.inlineParams();
	}
	/** @internal */
	getTransactionConfigStatements(config) {
		const statements = [];
		const chunks = this.getTransactionConfigChunks(config);
		if (chunks.length) statements.push(`set transaction ${chunks.join(" ")}`);
		if (typeof config.snapshot === "string") statements.push(this.dialect.sqlToQuery(this.setTransactionSnapshotSQL(config.snapshot)).sql);
		return statements;
	}
	setTransaction(config) {
		const chunks = this.getTransactionConfigChunks(config);
		const setOptions = chunks.length ? effect_Effect.asVoid(this.session.execute(__sql_sql_ts.sql.raw(`set transaction ${chunks.join(" ")}`))) : effect_Effect.void;
		if (typeof config.snapshot === "string") return effect_Effect.flatMap(setOptions, () => effect_Effect.asVoid(this.session.execute(this.setTransactionSnapshotSQL(config.snapshot))));
		return setOptions;
	}
};
