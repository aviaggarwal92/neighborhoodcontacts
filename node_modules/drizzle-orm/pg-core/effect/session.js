...
var PgEffectTransaction = class extends PgEffectDatabase {
	...
	setTransaction(config) {
		const chunks = this.getTransactionConfigChunks(config);
		const setOptions = chunks.length ? Effect.asVoid(this.session.execute(sql.raw(`set transaction ${chunks.join(" ")}`))) : Effect.void;
		if (typeof config.snapshot === "string") return Effect.flatMap(setOptions, () => Effect.asVoid(this.session.execute(this.setTransactionSnapshotSQL(config.snapshot))));
		return setOptions;
	}
};
