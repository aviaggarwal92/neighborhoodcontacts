...
import { NoDuplicateColumns, PgInsertBuilder } from "../query-builders/insert.cjs";
...
declare class PgEffectDatabase<TEffectHKT extends QueryEffectHKTBase, TQueryResult extends PgQueryResultHKT, TRelations extends AnyRelations = EmptyRelations> {
  ...
  execute<TRow extends unknown[] = unknown[]>(query: SQLWrapper | string, mode: 'arrays'): PgEffectRaw<TRow[], TEffectHKT>;
  execute<TRow extends Record<string, unknown> = Record<string, unknown>>(query: SQLWrapper | string, mode: 'objects'): PgEffectRaw<TRow[], TEffectHKT>;
  execute<TRow extends Record<string, unknown> = Record<string, unknown>>(query: SQLWrapper | string, mode?: 'raw' | undefined): PgEffectRaw<PgQueryResultKind<TQueryResult, TRow>, TEffectHKT>;
  transaction<A, E, R>(transaction: (tx: PgEffectTransaction<TEffectHKT, TQueryResult, TRelations>) => Effect.Effect<A, E, R>, config?: PgTransactionConfig): Effect.Effect<A, E | SqlError, R>;
}
