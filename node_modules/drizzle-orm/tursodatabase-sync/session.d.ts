...
import { Database, Transaction } from "@tursodatabase/sync";
...
declare class TursoDatabaseSyncSession<TRelations extends AnyRelations> extends SQLiteAsyncSession<'async', TursoDatabaseSyncRunResult, TRelations> {
  ...
  private logger;
  private cache;
  constructor(client: Database | Transaction, dialect: SQLiteDialect, relations: TRelations, options: TursoDatabaseSyncSessionOptions);
  ...
  transaction<T>(transaction: (db: TursoDatabaseSyncTransaction<TRelations>) => Promise<T>, config?: SQLiteTransactionConfig): Promise<T>;
}
