...
(function(_buildShape) {
	function fromTableOrView(source) {
		...
		const fields = orderSelectedFields(columns);
		const jsDbNameMap = is(source, Table) ? Object.fromEntries(Object.entries(columns).reduce((p, [k, c]) => {
			const { name } = c;
			if (k === name) return p;
			p.push([k, name]);
			return p;
