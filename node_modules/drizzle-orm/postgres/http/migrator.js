...
async function migrate(db, config) {
	...
	for (const migration of migrationsToRun) {
		for (const stmt of migration.sql) statements.push(db.execute(sql.raw(stmt)));
		statements.push(db.execute(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at", "name") values(${migration.hash}, ${migration.folderMillis}, ${migration.name})`));
	}
