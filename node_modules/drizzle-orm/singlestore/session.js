...
var SingleStoreDriverPreparedQuery = class extends SingleStorePreparedQuery {
	...
	async execute(placeholderValues = {}) {
		...
		return (this.rowMapper = this.rowMapper ?? (this.useJitMappers ? makeJitQueryMapper(fields, nullableObjectPaths) : makeDefaultQueryMapper(fields, nullableObjectPaths)))(rows);
	}
	...
	async *iterator(placeholderValues = {}) {
		...
		try {
			...
			while (true) {
				...
				else if (this.isRqbV2Query) if (this.useJitMappers) yield (this.rowMapper = this.rowMapper ?? makeJitRqbMapper(this.rqbConfig))([row]);
				else {
				...
				} else yield (this.rowMapper = this.rowMapper ?? (this.useJitMappers ? makeJitQueryMapper(fields, nullableObjectPaths) : makeDefaultQueryMapper(fields, nullableObjectPaths)))([row])[0];
				else yield row;
			}
		} finally {
