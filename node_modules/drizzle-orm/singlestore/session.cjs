...
var SingleStoreDriverPreparedQuery = class extends __singlestore_core_session_ts.SingleStorePreparedQuery {
	...
	async execute(placeholderValues = {}) {
		...
		const { fields, client, rawQuery, query, nullableObjectPaths, customResultMapper, returningIds, generatedIds } = this;
		...
		return (this.rowMapper = this.rowMapper ?? (this.useJitMappers ? (0, __utils_ts.makeJitQueryMapper)(fields, nullableObjectPaths) : (0, __utils_ts.makeDefaultQueryMapper)(fields, nullableObjectPaths)))(rows);
	}
	...
	async *iterator(placeholderValues = {}) {
		...
		try {
			...
			while (true) {
				...
				else if (this.isRqbV2Query) if (this.useJitMappers) yield (this.rowMapper = this.rowMapper ?? (0, __relations_ts.makeJitRqbMapper)(this.rqbConfig))([row]);
				else {
				...
				} else yield (this.rowMapper = this.rowMapper ?? (this.useJitMappers ? (0, __utils_ts.makeJitQueryMapper)(fields, nullableObjectPaths) : (0, __utils_ts.makeDefaultQueryMapper)(fields, nullableObjectPaths)))([row])[0];
				else yield row;
			}
		} finally {
