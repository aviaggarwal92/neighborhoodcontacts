...
import { DatabasePromise, Transaction } from "@tursodatabase/database-common";
...
declare class TursoDatabaseSession<TRelations extends AnyRelations> extends SQLiteAsyncSession<'async', TursoDatabaseRunResult, TRelations> {
  ...
  private logger;
  private cache;
  constructor(client: DatabasePromise | Transaction, dialect: SQLiteDialect, relations: TRelations, options: TursoDatabaseSessionOptions);
  ...
  transaction<T>(transaction: (db: TursoDatabaseTransaction<TRelations>) => Promise<T>, config?: SQLiteTransactionConfig): Promise<T>;
}
