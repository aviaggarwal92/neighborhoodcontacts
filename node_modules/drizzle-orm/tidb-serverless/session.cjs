...
function tidbBeginOptions(config) {
	if (!config) return void 0;
	if (config.accessMode !== void 0) throw new Error("Access mode transaction config is not supported by driver");
	if (config.withConsistentSnapshot !== void 0) throw new Error("Consistent snapshot transaction config is not supported by driver");
	switch (config.isolationLevel) {
		case void 0: return;
		case "read committed": return { isolation: "READ COMMITTED" };
		case "repeatable read": return { isolation: "REPEATABLE READ" };
		default: throw new Error(`Isolation level '${config.isolationLevel}' is not supported by driver`);
	}
}
