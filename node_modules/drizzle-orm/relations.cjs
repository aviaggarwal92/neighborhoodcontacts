...
function processRelations(tablesConfig, tables) {
	...
	for (const [sourceTableName, tableConfig] of Object.entries(tablesConfig)) for (const [relationFieldName, relation] of Object.entries(tableConfig.relations)) {
		...
		if (Object.prototype.hasOwnProperty.call(tableConfig.table[__table_ts.TableColumns], relationFieldName)) throw new Error(`${relationPrintName}: relation name collides with column "${relationFieldName}" of table "${tableConfig.name}"`);
...
function relationsFilterToSQL(table, filter, tableRelations = {}, tablesRelations = {}, depth = 0) {
	...
	for (const [target, value] of entries) {
		...
		switch (target) {
			...
			default: {
				if (Object.prototype.hasOwnProperty.call(table[__table_ts.TableColumns], target) && table[__table_ts.TableColumns][target]) {
...
function relationsOrderToSQL(table, orders) {
	if (typeof orders === "function") {
		...
		return require_entity.is(data, require_sql_sql.SQL) ? data : Array.isArray(data) ? data.length ? require_sql_sql.sql.join(data.map((o) => require_entity.is(o, require_sql_sql.SQL) ? o : require_sql_expressions_select.asc(o)), new require_sql_sql.StringChunk(`, `)) : void 0 : require_entity.is(data, require_column.Column) ? require_sql_expressions_select.asc(data) : void 0;
	}
	const entries = Object.entries(orders).filter(([_, value]) => value);
	if (!entries.length) return void 0;
	return require_sql_sql.sql.join(entries.map(([target, value]) => (value === "asc" ? require_sql_expressions_select.asc : require_sql_expressions_select.desc)(fieldSelectionToSQL(table, target))), new require_sql_sql.StringChunk(`, `));
}
function relationExtrasToSQL(table, extras, codecs, inJson) {
	...
	return {
		sql: subqueries.length ? require_sql_sql.sql.join(subqueries, new require_sql_sql.StringChunk(`, `)) : void 0,
		selection
	};
}
