...
type MsSqlInsertValue<TTable extends MsSqlTable, TColumnsList extends string[] | 'all' = 'all', TModel extends Record<string, any> = InferInsertModel<TTable>> = { [K in keyof TModel as TColumnsList extends 'all' ? K : Extract<K, TColumnsList[number]>]: TModel[K] | SQL | Placeholder } & {};
...
declare class MsSqlInsertBuilder<TTable extends MsSqlTable, TQueryResult extends QueryResultHKT, TPreparedQueryHKT extends PreparedQueryHKTBase, TOutput extends Record<string, unknown> | undefined = undefined, TColumnList extends string[] | 'all' = 'all'> {
  static readonly [entityKind]: string;
  private config;
  protected table: TTable;
  protected session: MsSqlSession;
  protected dialect: MsSqlDialect;
  constructor(table: TTable, session: MsSqlSession, dialect: MsSqlDialect, output?: SelectedFieldsOrdered, columnList?: string[]);
  values(value: MsSqlInsertValue<TTable, TColumnList>): MsSqlInsertBase<TTable, TQueryResult, TPreparedQueryHKT, TOutput>;
  values(values: MsSqlInsertValue<TTable, TColumnList>[]): MsSqlInsertBase<TTable, TQueryResult, TPreparedQueryHKT, TOutput>;
  /**
...
declare class MsSqlInsertBase<TTable extends MsSqlTable, TQueryResult extends QueryResultHKT, TPreparedQueryHKT extends PreparedQueryHKTBase, TOutput extends Record<string, unknown> | undefined = undefined, TDynamic extends boolean = false, TExcludedMethods extends string = never> extends QueryPromise<TOutput extends undefined ? QueryResultKind<TQueryResult, any> : TOutput[]> implements SQLWrapper {
  ...
  protected $table: TTable;
  private config;
  constructor(table: TTable, values: MsSqlInsertConfig['values'], session: MsSqlSession, dialect: MsSqlDialect, output?: SelectedFieldsOrdered, columnList?: string[]);
  toSQL(): Query;
...
//#endregion
export { AnyMsSqlInsert, MsSqlInsert, MsSqlInsertBase, MsSqlInsertBuilder, MsSqlInsertConfig, MsSqlInsertDynamic, MsSqlInsertPrepare, MsSqlInsertValue, MsSqlInsertWithout, NoDuplicateColumns };
