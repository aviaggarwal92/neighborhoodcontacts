...
var TBuffer = class {
	constructor() {
		return Type.Refine(Type.Unsafe({ type: "buffer" }), (value) => value instanceof Buffer, () => "not a Buffer");
	}
};
var TDate = class {
	constructor() {
		return Type.Refine(Type.Unsafe({ type: "date" }), (value) => value instanceof Date, () => "not a Date");
	}
};
...
var TBigIntString = class {
	constructor() {
		return Type.Refine(Type.Unsafe({ type: "bigint-string" }), (value) => {
...
var TUnsignedBigIntString = class {
	constructor() {
		return Type.Refine(Type.Unsafe({ type: "unsigned-bigint-string" }), (value) => {
			if (typeof value !== "string" || !/^\d+$/.test(value)) return false;
			const bigint = BigInt(value);
			if (bigint < 0n || bigint > CONSTANTS.INT64_MAX) return false;
			return true;
