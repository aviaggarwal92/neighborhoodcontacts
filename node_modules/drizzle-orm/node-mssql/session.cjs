...
var NodeMsSqlPreparedQuery = class extends __mssql_core_session_ts.PreparedQuery {
	...
	async execute(placeholderValues = {}) {
		...
		const { fields, client, rawQuery, nullableObjectPaths, customResultMapper } = this;
		...
		return (this.rowMapper = this.rowMapper ?? (this.useJitMappers ? (0, __utils_ts.makeJitQueryMapper)(fields, nullableObjectPaths) : (0, __utils_ts.makeDefaultQueryMapper)(fields, nullableObjectPaths)))(rows.recordset);
	}
	async *iterator(placeholderValues = {}) {
		const params = (0, __sql_sql_ts.fillPlaceholders)(this.params, placeholderValues);
		const { fields, rawQuery, nullableObjectPaths, client, customResultMapper } = this;
		let queryClient = client;
		if ((0, __entity_ts.is)(client, require_node_mssql_pool.AutoPool)) queryClient = await client.$instance();
		const request = queryClient.request();
		request.stream = true;
